...
class Model(nn.Module):
    def __init__(self, input_dim, demo_dim, gru_dim, graph_head, phi, mask_p, walks_per_node, walk_length,
                 gcn_dim, n_clusters, ffn_dim, del_rate, add_rate, tau, lambda_kl, lambda_cl, b1, b2, lambda1, dropout):
    ...
    def forward(self, x, x_demo, sorted_length):
        ...
        loss_dam = F.binary_cross_entropy(torch.sigmoid(adj_hat), new_adj)

        num_nodes = z_aug.shape[0]
        deg = degree(edge_index_aug[1].flatten(), num_nodes).float()
