...
class MaskPath(nn.Module):
    def __init__(self, p, walks_per_node, walk_length, start, num_nodes=None):
        super().__init__()
        self.p = p
    ...
    def forward(self, edge_index):
        remain_edge_index = self.mask_path(edge_index, self.random_walk, self.p, walks_per_node=self.walks_per_node,
                                           walk_length=self.walk_length, start=self.start, num_nodes=self.num_nodes)

        return remain_edge_index

    def mask_path(self, edge_index, random_walk, p=0.3, walks_per_node=1, walk_length=3, num_nodes=None,
                  start='node', is_sorted=False, training=True):
        ...
        walk = random_walk(row, col, start, walk_length)
        walk_tmp = torch.hstack([walk[:, i].unsqueeze(1).repeat(1, 2) for i in range(1, walk.shape[1] - 1)])
        walk_edge = torch.cat((walk[:, 0].unsqueeze(1), walk_tmp, walk[:, -1].unsqueeze(1)), 1)

        walk_edge_index = walk_edge.reshape(-1, 2).T
        walk_edge_index = torch.unique(walk_edge_index, dim=1)
        walk_edge_index, _ = remove_self_loops(walk_edge_index)

        edge_index_tuple = tuple(map(tuple, edge_index.T.cpu().detach().numpy()))
        walk_edge_index_tuple = tuple(map(tuple, walk_edge_index.T.cpu().detach().numpy()))
        remain_edge_index = set(edge_index_tuple) - set(walk_edge_index_tuple)
        remain_edge_index = torch.tensor(np.array(list(remain_edge_index))).T

        return remain_edge_index.to(device)
...
class Model(nn.Module):
    def __init__(self, input_dim, demo_dim, gru_dim, graph_head, phi, mask_p, walks_per_node, walk_length,
        ...
        self.clustering = Clustering(n_clusters, gcn_dim)
        self.ffn = nn.Linear(gcn_dim, ffn_dim)

        self.degree_decoder = DegreeDecoder(in_channels=gcn_dim, hidden_channels=12)
    ...
    def forward(self, x, x_demo, sorted_length):
        ...
        edge_index_aug = self.maskpath(edge_index)
        z = self.gcn(x_star, edge_index)
        z_aug = self.gcn(x_star, edge_index_aug)
        cluster_label, kl_loss = self.clustering(z)

        new_edge_index = del_edge(z, edge_index, cluster_label, self.del_rate)
        new_edge_index = add_edge(z, new_edge_index, cluster_label, self.add_rate)
        ...
        loss_hybrid = self.lambda_kl * kl_loss + self.lambda_cl * cl_loss
        # output = self.pre(z)

        adj_hat = torch.mm(z_aug, z_aug.T)
        loss_dam = F.binary_cross_entropy(torch.sigmoid(adj_hat), adj_init)

        num_nodes = maybe_num_nodes(edge_index, num_nodes=None)
        deg = degree(edge_index[1].flatten(), num_nodes).float()
        loss_mgm = self.b1 *loss_dam + self.b2 * F.mse_loss(self.degree_decoder(z_aug).squeeze(), deg)

        loss_pretrain = self.lambda1 * loss_mgm + (1 - self.lambda1) * loss_hybrid

        return z, loss_pretrain
