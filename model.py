...
def clloss_alpha(z, z_aug, adj, cluster_label, tau, hidden_norm=True):
    adj = adj - torch.diag_embed(adj.diag())
...
def clloss_beta(z, z_aug, adj, cluster_label, tau, hidden_norm=True):
    adj = adj - torch.diag_embed(adj.diag())
    ...
    positive = inter_view_sim.diag() + intra_view_sim.mul(adj).sum(1)

    loss = positive / (intra_view_sim.sum(1) + inter_view_sim.sum(1) - intra_view_sim.diag())

    adj_count = torch.sum(adj, 1) + 1
    loss = torch.log(loss) / adj_count

    return -torch.sum(loss, 0)


def clloss_gamma(z, z_aug, adj, cluster_label, tau, hidden_norm=True):
    adj = adj - torch.diag_embed(adj.diag())
    ...
    positive = inter_view_sim.diag() + intra_view_sim.mul(cluster_adj_new).sum(1)

    loss = positive / (intra_view_sim.sum(1) + inter_view_sim.sum(1) - intra_view_sim.diag())

    adj_count = torch.sum(cluster_adj_new, 1) + 1
    loss = torch.log(loss) / adj_count
